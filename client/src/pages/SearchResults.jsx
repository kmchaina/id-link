    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {!showAlert && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center justify-between gap-3">
          <p className="text-sm text-amber-800">Can't find your ID? Get an SMS alert when it's handed in.</p>
      ...
      {showAlert && (
        <div className="card mb-6 border-amber-200">
          <h3 className="font-bold mb-3">Alert Subscription — TZS 5,000</h3>
          {alertMsg ? (
            ...
            <form onSubmit={handleAlert} className="space-y-3">
              <input value={alertPhone} onChange={e => setAlertPhone(e.target.value)} placeholder="Your phone (e.g. +255700000000)" className="input" />
              <input value={alertId} onChange={e => setAlertId(e.target.value)} placeholder="Your full ID number" className="input" />
              <p className="text-xs text-gray-500">Payment of TZS 5,000 will be processed via mobile money.</p>
              <button type="submit" disabled={alertLoading} className="btn-primary text-sm py-2.5">
                {alertLoading ? 'Processing...' : 'Confirm & Pay TZS 5,000'}
              </button>
            </form>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">Try a different name or region, or set up an alert above.</p>
          <button onClick={() => navigate('/')} className="mt-4 text-brand-700 text-sm underline">← Back to home</button>
        </div>
      ) : (
        <>
